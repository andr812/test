...
racingtest(void)
{
  ...
  for (i = 0; i < NUM_THREAD; i++){
    if (thread_join(threads[i], &retval) != 0 || (int)retval != i+1){
      ...
      printf(1, "panic but thread: %d retval %d\n",(uint)threads[i],(int)retval);
      return -1;
    }
    printf(1,"success: i:%d  thread:%d  retval:%d\n", i, (uint)threads[i], (int)retval);
  }
  ...
  for (i = 0; i < NUM_THREAD; i++){
    if(thread_join(threads[i],&retval) == 0){
      printf(1,"thread: %d i %d retval %d\n",(uint)threads[i], i,(int)retval);
    }
  }
  */
  printf(1,"%d\n", gcnt);
